...
function calcBid(bidList) {
    ...
    $('#menor').text('El precio más bajo: R$:' + menorBid)
    let maiorBid = bidList.sort((a, b) => a - b).at(-1)
    $('#maior').text('Cotización más alta: R$:' + parseFloat(maiorBid).toFixed(2) )
    let variacaoBid = (((maiorBid - menorBid) / menorBid) * 100).toFixed(2)
    $('#variacao').text('Variación de período: ' + variacaoBid+"%")
    let bidMedio = (bidTotal / bidList.length).toFixed(2)
    $('#media').text('Promedio en el período: R$' + bidMedio)
...
function signOut() {
    ...
    auth2.signOut().then(function () {
        document.getElementById('dropText').innerText = "Registrase / Login"
        let logImg = document.getElementById('loginImg')
...
<div class="row d-flex justify-content-between" style="background-color: #F1BE07; color:#1F6D69">
    ...
    <div class="col-lg-2 col-md-4 col-sm-6 font-weight-bold">Variación de 360 ​​días:</div>
    ...
    <div class="container-fluid text-warning" style="background-color: #1F6D69;">
        <div class="row">
            ...
            <div class="col-lg-4 col-md-12-d my-3">
                <div class="row d-flex justify-content-center">HABLE CON NOSOTROS</div>
        ...
        <div class="row d-flex justify-content-center" style="background-color: #1F6D69; color:"#F1BE07">
         Copyright © 2021 Queops Maritime | 
         Todos los derechos reservados | Políticas de
            Intimidad | Políticas de
             Cookies
            </div>
        </div>
    `
...
function tratarCNPJ(dado) {
    ...
    if (dadosCNPJ.situacao != "ATIVA") {
        ...
        $('#alerta').text('Cotizaciones solo para CNPJs activos')
            abrirModalAlerta()
    } else {
...
function validarForm() {
    if ($("#razaoSocial").val().length < 3) {
        $('#alerta').text('Complete el formulario correctamente')
            abrirModalAlerta()
